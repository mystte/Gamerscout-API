...
var sendValidateAccountEmail = function(email, host, token) {
  ...
  var mailOptions = {
    ...
    text:
      "You are receiving this email because you (or someone else) have created an account on Gamerscout.\n\n" +
...
const sendValidatePasswordEmail = (email, host, token) => {
  ...
  var mailOptions = {
    ...
    text:
      "You are receiving this email because you (or someone else) have requested a new password for your Gamerscout account.\n\n" +
      "Please click on the following link, or paste this into your browser to complete the validation process:\n\n" +
      host +
      "/p/" +
      token +
...
var sendUpdateEmailNotification = function(email, host) {
  ...
  var mailOptions = {
    ...
    text:
      "You are receiving this email because you (or someone else) have updated your gamerscout email.\n\n" +
...
router.post("/validate_password", async function(req, res, next) {
  if (!req.session.email && !req.session._id)
    return res.status(403).json({ error: "errAuthenticationRequired" });
  const userEmail = req.session.email;
  const password = req.body.password ? req.body.password : null;

  const currentUser = await User.findOne({
    $or: [{ email: userEmail }, { facebookEmail: userEmail }]
  });

  if (!currentUser) return res.status(400).json({ error: "errUserNotFound" });

  const isMatch = await currentUser.comparePassword(
    password,
...
router.post("/add_facebook", async function(req, res, next) {
  ...
  const facebookProfile = JSON.parse(
    await request(
      config.facebook_url + "/" + config.facebook_url_profile + token
    )
  );
      ...
      const loggedUser = await User.findOne(
        {
          $or: [
            ...
            { facebookEmail: req.session.email }
          ]
        },
        { twitter_id: 0, __v: 0 }
      );
...
router.post("/facebook_auth", function(req, res, next) {
  ...
  request(
    config.facebook_url + "/" + config.facebook_url_profile + access_token
    ...
    request(
      config.facebook_url +
        "/" +
        result_json.id +
        config.facebook_url_picture +
        access_token
    )
      .then(function(picture_result) {
        // Get facebook profile picture
        var picture_json = JSON.parse(picture_result);
        if (result_json.email && result_json.id) {
          var uri =
            req.protocol + "://" + req.header("host") + "/api/1/users/login";
          return Q()
            .then(function() {
              return User.findOne(
                {
                ...
                return Q()
                  ...
                  .then(function() {
                    if (environment === "production")
                      slack.slackNotificationSubscriptions(
                        "Congratulation boyz!!! You have a new gamer in your community : `" +
                          username +
                          "`"
                      );
                    ...
                    sendValidateAccountEmail(
                      result_json.email,
                      req.protocol + "://" + constants.CLIENT_BASE_URL,
                      newUser.validateAccountToken
                    );
                    return res.status(201).json({
                      ...format_login_export(user_json),
                      "gamerscout-api-session":
                        req.cookies["gamerscout-api-session"]
                    });
                  });
              } else {
...
router.post("/facebook_disconnect", async function(req, res) {
  ...
  if (!foundUser.facebookEmail)
    return res.status(400).json({ error: "errNoFacebookEmail" });
  if (foundUser.isAutomaticGeneratedPwd)
    return res.status(400).json({ error: "errPasswordUpdateRequired" });
  if (foundUser.facebookEmail) {
...
router.post("/validation/email/resend", function(req, res, next) {
  if (!req.session.email && !req.session._id)
    return res.status(403).json({ error: "authentication required" });
  User.findOne({
    $or: [{ email: req.session.email }, { facebookEmail: req.session.email }]
  }).then(user => {
    if (!user) {
      res.status(404).json({ error: "User not found" });
    } else {
      const receiverEmail = user.emailToValidate
        ? user.emailToValidate
        : req.session.email;
          ...
          newUser
            ...
            .then(function(createdUser) {
              ...
              sendValidateAccountEmail(
                createdUser.email,
                req.protocol + "://" + constants.CLIENT_BASE_URL,
                createdUser.validateAccountToken
              );
            ...
            .catch(function(error) {
              ...
              if (error.name === "ValidationError") {
                if (error.errors.email)
                  return res.status(400).json({ error: "errWrongEmail" });
                if (error.errors.username)
                  return res.status(400).json({ error: "errWrongUsername" });
                if (error.errors.password)
                  return res.status(400).json({ error: "errWrongPassword" });
              }
              if (error.code === 11000)
                return res.status(400).json({ error: "errUserExists" });
...
router.delete("/delete/:user_id", function(req, res, next) {
  ...
  var app_id = req.headers["x-api-client-id"]
    ? req.headers["x-api-client-id"]
    : null;
...
router.post("/forgotten_password", function(req, res, next) {
  ...
  if (email) {
    User.findOne({ $or: [{ email }, { facebookEmail: email }] }, function(
      err,
      user
    ) {
      if (err) {
        ...
        crypto.randomBytes(20, function(err, buf) {
          ...
          found_user.save();
          logic_forgot_password.send_forgot_password_email(
            email,
...
router.post("/:user_id/avatar", upload.single("avatar"), function(
  req,
  res,
  next
) {
        ...
        return Q()
          ...
          .then(function(user, err) {
            if (err) {
              ...
              res
                .status(403)
                .json({ error: "Trying to modify another user's profile" });
            } else {
              user.avatar =
                req.protocol +
                "://" +
    ...
    return Q()
      ...
      .then(async function(user, err) {
        if (err) {
          ...
          user.facebookEmail === req.session.email
        ) {
